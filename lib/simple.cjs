   ...
   async value (ms) {
	...
	let type = await fileTypeFromBuffer(buffer)
       let trueFileName = attachExtension ? (filename + '.' + type.ext) : filename
        ...
        sendListButton: {
        async value(jid, text, list, footer, options = []) {
...
conn.relayMessage(jid, msg.message, {
  messageId: msg.key.id,
       ...
       sendUrlButton: {
        async value(jid, text, row = [], footer) {
	let msg = generateWAMessageFromContent(jid, {
  viewOnceMessage: {
    message: {
        "messageContextInfo": {
...
conn.relayMessage(jid, msg.message, {
  messageId: msg.key.id,
        ...
        sendUrlImageButton: {
        async value(jid, text, row = [], footer, url) {
	let msg = generateWAMessageFromContent(jid, {
  viewOnceMessage: {
    message: {
        "messageContextInfo": {
...
conn.relayMessage(jid, msg.message, {
  messageId: msg.key.id,
        ...
        sendListImageButton: {
        async value(jid, text, list, footer, url, options = []) {
	let msg = generateWAMessageFromContent(jid, {
  viewOnceMessage: {
    message: {
        "messageContextInfo": {
...
let pathFile1 = await writeExif(media, { packname: options.packname ? options.packname : 'LitoBot - MD', author: options.author ? options.author : '', categories: options.categories ? options.categories : [] })
